...
const AUTH_KEY = "immi.auth.v1";

/* The shared endpoint costs money per call, so it is the one provider that
   requires a signed-in account. A BYO-key provider spends the visitor's own
   credit and stays open. */
// Learned from the endpoint (see aiRefreshAuth). Defaults to "open": if the
// lookup has not landed yet, a visitor gets the tool rather than a login prompt
// that may not even apply. Guessing wrong this way costs one clear 401; the
// other way it turns an open feature into a locked one.
let AI_REQUIRE_AUTH = false;
...
const PROVIDERS = {
  ...
  public: { label: "公共服务（本站代付）", api: "proxy", base: "",
            models: ["（由服务端固定）"],
            note: "调用费用由站点方承担，所以每天有次数额度。" +
                  "想不受额度限制，就在下面换成自己的 Key。" },
...
async function aiSend() {
  ...
  if (aiProv().api === "proxy" && AI_REQUIRE_AUTH && !aiAuth()) {
    stat.textContent = "公共服务需要先用邮箱登录。点上方的「去登录」，" +
      "或在第 1 步换成自己的 API Key。";
    aiRenderGate();
    return;
  }
    ...
    if (d && d.quota) {
      ...
      if (a) {
        try { localStorage.setItem(AUTH_KEY, JSON.stringify(
          Object.assign({}, a, { plan: d.plan || a.plan,
                                 used: d.quota.used, perDay: d.quota.perDay }))); } catch (e) {}
        aiRenderGate();
      }
    }
...
async function aiRenderResult(x) {
  ...
  // Points are published in 5-point bands, and the top bands often do not
  // exist for an occupation at all -- at 100 points most cells in this table
  // fall back. Printing 2.4% under a "105分" header when the figure is really
  // the 95-point band is the kind of quiet substitution that makes a whole
  // table untrustworthy, so name the band that was actually used.
  const cell = r => !r ? "—"
    : `${r.rate}%` + (r.exact ? "" : `（${r.at}分）`) + (r.n < 50 ? "▲" : "");
  ...
  const approx = withRates.reduce((a, w) =>
    a + cols.filter(c => w.per[c] && !w.per[c].exact).length, 0);
  note.textContent = `按你在「计算分数」页算出的 ${base} 分排序：189 查 ${base} 分、` +
    `190 查 ${base + 5} 分、491 查 ${base + 15} 分（后两者含州/地区提名加分）。` +
    "每格是「正好在那一档」的人里历史上获邀的比例，不是「≥该分数」的累计值。" +
    "官方按 5 分一档发布，所以高分档常常没有数据；括号里标出实际取的是哪一档，" +
    "读的时候要当成下限。带 ▲ 的样本不足 50，不可靠。每行最高的一格加粗，" +
    "点职业名可查看完整数据页。" +
    (approx ? `本表 ${approx} 格因为你那一档无数据而落回了更低档。` : "") +
    (dropped.length ? `模型还提到了 ${dropped.join("、")}，不在 SkillSelect 清单上，已略过。` : "");
}

/* The bar at the top of the AI view: signed out it explains why the shared
   service needs an account and offers the way in; signed in it shows who you
...
function aiRenderGate() {
  ...
  if (!bar) return;
  bar.textContent = "";
  const proxy = aiProv().api === "proxy";
  if (!proxy || !PUBLIC_PROXY_URL) { bar.hidden = true; return; }
  bar.hidden = false;

  const a = aiAuth();
  if (!a && !AI_REQUIRE_AUTH) { bar.hidden = true; return; }   // open to everyone
  const span = (t, cls) => {
  ...
  const btn = (t, fn) => {
    const b = document.createElement("button");
    b.type = "button"; b.className = "tblbtn nt"; b.textContent = t;
    b.addEventListener("click", fn);
    return b;
  };

  if (!a) {
    bar.append(span("公共服务要花钱调模型，所以需要邮箱验证登录。1、2 两页不用登录。"),
               span("", "grow"),
               btn("去登录", () => { location.href = "login.html" + location.hash; }));
    return;
  }
  const left = a.perDay != null && a.used != null ? Math.max(0, a.perDay - a.used) : null;
  bar.append(span(a.plan === "paid" ? "付费" : "免费", "pill"),
             span(a.email || "已登录"),
             span(left == null ? "" : `今天还剩 ${left} / ${a.perDay} 次`),
             span("", "grow"),
             btn("退出登录", () => {
               aiSignOut();
               aiRenderGate();
               $ai("aiSendStat").textContent = "已退出登录。";
             }));
}

/* Ask the endpoint who we are: the plan and the remaining quota are looked up
   there, never trusted from the copy sitting in localStorage. */
async function aiRefreshAuth() {
  if (!PUBLIC_PROXY_URL) return;
  const a = aiAuth();
  try {
    const r = await fetch(PUBLIC_PROXY_URL.replace(/\/+$/, "") + "/auth/me", {
      method: "POST",
    ...
    const d = await r.json().catch(() => ({}));
    AI_REQUIRE_AUTH = !!d.requireAuth;
    if (d.maxChars) PROXY_MAX_CHARS = d.maxChars;
    if (!d.signedIn) {
      if (a) aiSignOut();          // the token we held is no longer good
      aiRenderGate();
      return;
    }
    const next = Object.assign({}, a, { email: d.email, plan: d.plan,
                                        perDay: d.quota && d.quota.perDay,
                                        used: d.quota && d.quota.used });
