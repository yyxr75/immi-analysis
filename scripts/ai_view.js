...
const PROVIDERS = {
  ...
  public: { label: "公共服务（本站代付 · 需邮箱登录）", api: "proxy", base: "",
            models: ["（由服务端固定）"],
            note: "调用费用由站点方承担，所以要邮箱验证登录、每天有额度。" +
                  "不想登录、或想不受额度限制，就在下面换成自己的 Key。" },
...
async function aiRefreshAuth() {
  ...
  try {
    ...
    const d = await r.json().catch(() => ({}));
    if (!d.signedIn) { aiSignOut(); aiRenderGate(); return; }
