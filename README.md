...
**排序用获邀率，不用语义相似度。** 只按模型的贴近程度排，结果永远是最难的那几个
（Software Engineer 语义最准，同时也是 95 分线、在池 5.6 万）。所以：模型的判断只当
**种子**，相关职业按 ANZSCO 代码层级推出（同一四位小类 → 三位中类 → 两位大类，上限 14 个），
再用 `CALC` 里用户实际算出的分数去查每个职业的历史获邀率，从高到低排。
189 用基础分、190 用 +5、491 用 +15。没填过分数就提示先去打分。

这一步在措辞上必须守住边界：**获邀率高 ≠ 你能用这个职业申请**。能不能用取决于职业评估机构
是否认可你的背景，页面上对此有明确警示。
